...
const login = async (request, response) => {
  ...
  if (user && user.verified) {
    ...
    if (passwordMatch) {
      // Generar token de autenticación (si es necesario)
      ...
      return response.render('auth/login', {
        ...
        user: { email } // Puedes enviar el correo electrónico para mantenerlo lleno en el formulario en caso de error
      });
    }
  } else {
    // Usuario no existe o no está verificado
    return response.render('auth/login', {
      authenticate: true,
      page: "Login",
      errors: [{ msg: 'Invalid user or user not verified' }],
      user: { email }
    });
  }
};
...
const registerAccount = async (request, response) => {
  console.log(
    "El usuario está intentando registrar sus datos en la base de datos"
  );
  await check("name")
    .notEmpty()
  ...
  await check("email")
    ...
    .isEmail()
    .withMessage(
      "This field should be an Email (user@domain.ext) and not empty"
    )
    .run(request);
  ...
  await check("password")
    ...
    .isLength({ min: 8 })
    .withMessage("Password must contain at least of 8 characters")
    .isLength({ max: 20 })
    .withMessage("Password must contain less than 20 characters")
    .equals(request.body.repeatPassword)
  ...
  if (result.isEmpty()) {
    ...
    console.log(
      `Intentando insertar al usuario: ${name}, con correo electrónico: ${email}, password: ${password} y token: ${token}`
    );
...
const resetPassword = async (request, response) => {
  await check("email")
    ...
    .isEmail()
    .withMessage(
      "The Email field should be an Email (user@domain.ext) and not empty"
    )
    .run(request);
